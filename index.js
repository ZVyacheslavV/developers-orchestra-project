import{i as y,a as m,g}from"./assets/vendor-BAa1Uqhj.js";(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))i(s);new MutationObserver(s=>{for(const a of s)if(a.type==="childList")for(const n of a.addedNodes)n.tagName==="LINK"&&n.rel==="modulepreload"&&i(n)}).observe(document,{childList:!0,subtree:!0});function r(s){const a={};return s.integrity&&(a.integrity=s.integrity),s.referrerPolicy&&(a.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?a.credentials="include":s.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function i(s){if(s.ep)return;s.ep=!0;const a=r(s);fetch(s.href,a)}})();const o={artistsList:document.querySelector("#artists-list"),artistDetailsModal:document.querySelector(".artist-details-modal"),artistDetailsModalCloseBtn:document.querySelector(".artist-details-modal-close-btn"),menuGenres:document.querySelector(".dropdown-menu-genres"),menuSort:document.querySelector(".dropdown-menu-sort"),btnGenres:document.querySelector(".dropdown-toggle-genres"),btnSort:document.querySelector(".dropdown-toggle-sort")},w="https://sound-wave.b.goit.study/api",u={ARTISTS:"/artists",ARTIST_BY_ID:"/artists/",ARTIST_ALBUMS_BY_ID:"/albums",GENRES:"/genres",FEEDBACKS:"/feedbacks",PRODUCTS_BY_CATEGORY:"/products/category/"},$=8;function f(e,t="center"){y.error({title:"Error",message:e,position:t})}const S={currentPage:1};m.defaults.baseURL=w;const E=async e=>{const{data:t}=await m.get(`${u.ARTISTS}?limit=${$}&page=${e}`);return t},A=async e=>{const{data:t}=await m.get(`${u.ARTIST_BY_Id}${e}`);return t},T=async e=>{const{data:t}=await m.get(`${u.ARTIST_BY_Id}${e}${u.ARTIST_ALBUMS_BY_ID}`);return t},I=async()=>{const{data:e}=await m.get(`${u.GENRES}`);return e};async function M(){try{const{artists:e}=await E(S.currentPage);k(e)}catch{f("Failed to fetch artists")}}function k(e=[]){const t=e.map(({_id:r,strArtist:i="Unknown",strArtistThumb:s,strBiographyEN:a="",genres:n=[]})=>`
    ...
    `,a=o.artistDetailsModal.querySelector(".container");a.innerHTML=`
      ...
      ${s}
    `,_(),B()}catch(t){y.error("Error loading artist:",t)}finally{C()}}o.artistsList.addEventListener("click",e=>{const t=e.target.closest(".artist-cta");if(!t)return;const r=t.dataset.artistId;r&&R(r)});
